...
class Quick_Flickr_Widget extends WP_Widget {
  ...
  public function widget( $args, $instance ) {
    ...
    if ( is_wp_error( $photos ) ) {
      ...
      foreach ( $photos as $photo ) {
        ...
        $source = str_replace('_m', '_q', $src); // Swap to medium size 
        ...
        $item = sprintf( '<a href="%s"><img src="%s" alt="%s" /></a>', $link, $source, $title );
        $item = sprintf( '<li class="' .$classv. '"><div class="quick-flickr-item">%s</div></li>', $item );
        echo $item;
      }
    }
    echo "</ul>";
  ...
  public function form( $instance ) {
    ...
    <p>
      <label for="<?php echo $this->get_field_id( 'username' ); ?>"><?php _e( 'Username or RSS:', 'nadia' ); ?></label> 
      <input class="widefat" id="<?php echo $this->get_field_id( 'username' ); ?>" name="<?php echo $this->get_field_name( 'username' ); ?>" type="text" value="<?php echo esc_attr( $username ); ?>" />
    </p>
    <p>
      <label for="<?php echo $this->get_field_id( 'tags' ); ?>"><?php _e( 'Tags:', 'nadia' ); ?></label> 
    ...
    <p>
      <label for="<?php echo $this->get_field_id( 'count' ); ?>"><?php _e( 'Count:', 'nadia' ); ?></label><br />
...
function quick_flickr_widget_upgrade() {
  ...
  $new[] = array(
    ...
    'count' => isset( $old['items'] ) ? absint( $old['items'] ) : 6,
